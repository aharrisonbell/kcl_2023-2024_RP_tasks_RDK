...
% EITHER the correct direction of motion OR the correct RESPONSE button (in separate between-subjects design). 
% With each progressive block, the participants are given increasing clues as to the correct relationship
% between the dot colour and the direction of motion.
%% PRACTICE VERSION

%% Trial/Task Description
% 144 Trials per block; total trials = 144 * 4
% Block 1: 4 directions (UL, LL, UR, LR) * 6 coherence levels (5%-50%)
...
for currBlock = startingBlock:total_number_of_blocks % allows for manually starting at any block
    if currBlock == 1 % display instructions for block 1
        ...
        instructionimg = imread('kcl_rdk_Shuyi_PromptScreens_practice.jpg');
